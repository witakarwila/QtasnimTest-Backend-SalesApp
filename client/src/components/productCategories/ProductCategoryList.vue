<template>
    <div class="min-h-full">
      ...
      <header class="bg-white shadow">
        <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
          <h1 class="text-3xl font-bold tracking-tight text-gray-900">Product Category</h1>
        </div>
      </header>
      <main>
        <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
          <div class="relative flex flex-row justify-between items-center my-10">
            <router-link :to="{ name: 'Add Product Category' }" class="rounded-lg text-lg px-5 py-3 bg-[#000] text-[#ffffff] justify-center me-3">
                  + Add New
            </router-link>
          </div>
          <div class="relative shadow-md rounded-lg">
              <table class="table-fixed w-full text-left">
                <thead class="uppercase bg-[#000000] text-[#fafafa] h-16">
                      <tr>
                          <td class="py-1 border text-center  p-4">Category Name</td>
                  ...
                  <tbody class="bg-white text-gray-500 bg-[#FFFFFF] text-[#6b7280]" style="background-color: #FFFFFF; color: #6b7280;">
                      <tr v-for="item in items" :key="item.id" class="py-5">
                          ...
                          <td class="py-5 border text-center  p-4">
                            <router-link :to="{ name : 'Edit Product Category', params: {id: item.id} }" class="rounded-lg text-lg px-5 py-2 bg-[#0062ff] text-[#ffffff] justify-center me-3">
                                Edit
                            </router-link>
                            <a @click="deleteJenisBarang(item.id)" class="rounded-lg text-lg px-5 py-2 bg-[#ff4b4b] text-[#ffffff] justify-center">Delete</a>
                          </td>
                      </tr>
                  </tbody>
              </table>
  ...
  export default {
    ...
    methods: {
      ...
      async deleteJenisBarang(id) {
        try {
          await axios.delete(`http://localhost:5000/api/v1/JenisBarang/${id}`);
          this.getAll();
        } catch (err) {
          console.log(err);
        }
